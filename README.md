...
这是一个面向多职场人员全流程跟踪的人力资源管理平台，覆盖候选人 H5 自助填报、面试、入职、培训、在职、离职、项目权限、自动招聘统计、日报快照、PDF/Excel 导出和操作日志。
...
- 状态约束：未到面候选人标记通过时需二次确认；确认入职后进入培训，培训结束后转为在职，培训中和在职人员办理离职时必须填写日期和原因。
- 自动统计：新增、到面、通过、入职、离职均根据候选人状态时间自动计算，无需 HR 每日录入。
- 月度人力：当前在岗仅统计培训结束后的在职员工，培训人员单独统计；剩余缺口 = 目标总人数 - 当前在岗。HR 仅需按月确认目标总人数。
